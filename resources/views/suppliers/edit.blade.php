...
<div class="max-w-xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold text-[#1d1d1f] tracking-tight">Edit Supplier</h1>
            <p class="text-xs text-[#86868b] mt-1">Perbarui kontak dan alamat pemasok</p>
        </div>
    ...
    <div class="apple-card p-6">
        <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-[#1d1d1f] mb-1.5">No. Telepon</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-[#1d1d1f] mb-1.5">Alamat</label>
                <textarea name="alamat" rows="3"
                    class="apple-input h-auto py-2">{{ old('alamat', $supplier->alamat) }}</textarea>
            </div>
