...
<div class="max-w-xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl sm:text-2xl font-semibold text-[#1d1d1f] tracking-tight">Tambah Pengguna Baru</h1>
            <p class="text-xs text-[#86868b] mt-1">Buat akun untuk staf kasir, manajer, atau admin</p>
        </div>
        ...
        <form method="POST" action="{{ route('users.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                    class="apple-input @error('name') border-[#d70015] @enderror" placeholder="Nama pengguna">
            </div>
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" required
                    class="apple-input @error('email') border-[#d70015] @enderror" placeholder="user@email.com">
            </div>

            <div>
                <label for="password" class="block text-xs font-semibold text-[#1d1d1f] mb-1.5">Kata Sandi <span class="text-[#d70015]">*</span></label>
                <input type="password" id="password" name="password" required minlength="3"
                    class="apple-input @error('password') border-[#d70015] @enderror" placeholder="••••••••">
            </div>
            ...
            <div>
                ...
                <select id="role" name="role" required class="apple-input">
                    <option value="staff" {{ old('role') === 'staff' ? 'selected' : '' }}>Staff Kasir (Hanya POS & Riwayat)</option>
                    <option value="manager" {{ old('role') === 'manager' ? 'selected' : '' }}>Manager (Produk, Stok, Pengeluaran & Laporan)</option>
